...
export default function DashboardAppPage() {
  ...
  onValue(parkingArrayRef, async (snapshot) => {
    ...
    if (!dataParkingArrayRef) {
      ...
      totalRoom = dataParkingArrayRef.length;
      totalCost = dataParkingArrayRef.reduce((total, item) => {
        const currentValue = calculateParkingCost(item?.date, item?.time);
      ...
      <Container maxWidth="xl">
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} sm={6} md={6}>
            <AppWidgetSummary title="Số phòng đang dùng" total={totalRoom} icon={'cil:room'} />
          </Grid>
